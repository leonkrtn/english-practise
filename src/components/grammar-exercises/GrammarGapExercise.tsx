...
export default function GrammarGapExercise({ rule, onAnswered, onNext }: GrammarExerciseProps) {
  ...
  function check(skipped: boolean) {
    ...
    const cls = classifyAnswer(raw, [rule.gap.answer]);
  ...
  return (
    <>
      ...
      <div className="text-[16px] leading-relaxed mb-3">
        {before}
        <span className="inline-block min-w-[70px] border-b-2 border-blue text-blue font-semibold text-center">{result ? rule.gap.answer : "?"}</span>
        {after}
      </div>
      ...
      {showHint && <div className="text-[13px] text-blue-dark bg-blue-light rounded-lg px-3 py-2 mt-2">{rule.gap.hint}</div>}
      {!result && (
        <ExerciseFooter>
