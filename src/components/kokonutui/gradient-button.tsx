...
/** Prefixes every class in a Tailwind class-list string with `dark:` — the upstream component
 * passes its "dark" gradient stops as plain classes, which apply unconditionally instead of only
 * under dark mode. */
function darkVariant(classes: string): string {
  return classes
    .split(" ")
    .map((c) => `dark:${c}`)
    .join(" ");
...
const gradientColors: Record<ColorVariant, GradientColors> = {
  blue: {
    dark: {
      border: "from-[#1e5fb4] via-[#0C1F21] to-[#0058b8]",
      overlay: "from-[#0071e3]/40 via-[#0C1F21] to-[#0058b8]/30",
      accent: "from-[#eaf3ff]/10 via-[#0C1F21] to-[#0d2a4a]/50",
      text: "from-[#cfe6ff] to-[#9cc9ff]",
      glow: "rgba(0,113,227,0.1)",
      textGlow: "rgba(0,113,227,0.4)",
      hover: "from-[#0d2a4a]/20 via-[#4da0ff]/10 to-[#0d2a4a]/20",
    },
    light: {
      border: "from-blue via-blue to-blue-light",
      base: "from-blue-lighter via-blue-lighter/80 to-blue-lighter/90",
      overlay: "from-blue-light/40 via-blue-lighter/30 to-blue-light/30",
      accent: "from-blue/20 via-blue-light/20 to-blue-lighter/30",
      text: "from-blue-dark to-blue",
      glow: "rgba(0,113,227,0.2)",
      hover: "from-blue-light/40 via-blue-lighter/20 to-blue-light/40",
    },
  },
  emerald: {
    dark: {
    ...
    >
      <div
        className={cn(
          ...
          "dark:bg-none",
          colors.light.border,
          darkVariant(colors.dark.border)
        )}
      >
        <div
          className={cn(
      ...
      <div
        className={cn(
          ...
          darkVariant(colors.dark.overlay)
        )}
      />
      <div
        className={cn(
          ...
          colors.light.accent,
          darkVariant(colors.dark.accent)
        )}
      />
