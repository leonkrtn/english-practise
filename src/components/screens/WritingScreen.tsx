"use client";

import { useMemo, useState } from "react";
import { AlertCircle, Check, Loader2, RefreshCw, Sparkles, X } from "lucide-react";
import type { Word } from "@/lib/vocab";
import type { GrammarRule } from "@/lib/grammar-data";
import type { WritingTopic } from "@/lib/writingTopics";
import { checkText, LanguageToolError, type LanguageToolMatch } from "@/lib/languageTool";
import { textLikelyUsesGrammar, textUsesWord } from "@/lib/grammarUsageCheck";

const MIN_WORDS = 40;

export interface WritingCheckResult {
  text: string;
  requiredWordsUsed: number;
  requiredWordsTotal: number;
  issues: LanguageToolMatch[];
}

export default function WritingScreen({
  topic,
  requiredWords,
  requiredRules,
  onExit,
  onFinish,
}: {
  topic: WritingTopic;
  requiredWords: Word[];
  requiredRules: GrammarRule[];
  onExit: () => void;
  onFinish: (result: WritingCheckResult) => void;
}) {
  const [text, setText] = useState("");
  const [checking, setChecking] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [matches, setMatches] = useState<LanguageToolMatch[] | null>(null);

  const wordCount = useMemo(() => (text.trim() ? text.trim().split(/\s+/).length : 0), [text]);
  const wordsUsed = useMemo(() => requiredWords.map((w) => textUsesWord(text, w)), [text, requiredWords]);
  const grammarUsed = useMemo(() => requiredRules.map((r) => textLikelyUsesGrammar(text, r)), [text, requiredRules]);
  const allWordsUsed = wordsUsed.every(Boolean);

  async function handleCheck() {
    if (checking) return;
    setChecking(true);
    setError(null);
    try {
      const result = await checkText(text);
      setMatches(result.matches);
    } catch (e) {
      setError(e instanceof LanguageToolError ? e.message : "Die Prüfung ist fehlgeschlagen. Bitte versuche es erneut.");
    } finally {
      setChecking(false);
    }
  }

  function handleFinish() {
    onFinish({
      text,
      requiredWordsUsed: wordsUsed.filter(Boolean).length,
      requiredWordsTotal: requiredWords.length,
      issues: matches || [],
    });
  }

  return (
    <section className="h-full flex flex-col">
      <div className="flex items-center gap-3 mb-3 shrink-0">
        <button
          onClick={onExit}
          title="Schreiben beenden"
          className="w-8 h-8 rounded-full border border-line bg-card text-ink-soft flex items-center justify-center shrink-0 transition-all hover:bg-line-soft hover:-translate-y-0.5 hover:shadow-sm"
        >
          <X size={16} />
        </button>
        <div className="text-[13px] font-semibold text-ink-soft">Schreibaufgabe</div>
      </div>

      <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain [-webkit-overflow-scrolling:touch] bg-card border border-line-soft rounded-[22px] p-5 shadow-[0_2px_8px_rgba(15,23,42,0.05),0_24px_48px_-18px_rgba(15,23,42,0.18)] flex flex-col animate-fade-in">
        <span className="inline-flex items-center gap-1 self-start rounded-full px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide mb-3 text-white shadow-sm bg-gradient-to-r from-amber to-amber-dark">
          Thema
        </span>
        <div className="text-[17px] font-bold tracking-tight mb-1 leading-snug">{topic.prompt}</div>
        <div className="text-[13px] text-ink-faint mb-4">{topic.hint}</div>

        <div className="mb-2 text-[11.5px] uppercase tracking-wide font-bold text-ink-faint">
          Pflichtwörter ({wordsUsed.filter(Boolean).length}/{requiredWords.length})
        </div>
        <div className="flex flex-wrap gap-1.5 mb-4">
          {requiredWords.map((w, i) => (
            <span
              key={w.id}
              className={
                "inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[12.5px] font-semibold border-[1.5px] transition-colors " +
                (wordsUsed[i] ? "border-green bg-green-light text-[#0d7a4f]" : "border-line bg-bg text-ink-soft")
              }
            >
              {wordsUsed[i] ? <Check size={12} /> : null}
              {w.en}
            </span>
          ))}
        </div>

        {requiredRules.length > 0 && (
          <>
            <div className="mb-2 text-[11.5px] uppercase tracking-wide font-bold text-ink-faint">Grammatik (versuche, das zu verwenden)</div>
            <div className="flex flex-wrap gap-1.5 mb-4">
              {requiredRules.map((r, i) => (
                <span
                  key={r.id}
                  className={
                    "inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[12.5px] font-semibold border-[1.5px] transition-colors " +
                    (grammarUsed[i] ? "border-purple bg-purple-light text-purple" : "border-line bg-bg text-ink-soft")
                  }
                >
                  {grammarUsed[i] ? <Check size={12} /> : null}
                  {r.title}
                </span>
              ))}
            </div>
          </>
        )}

        <textarea
          value={text}
          onChange={(e) => {
            setText(e.target.value);
            setMatches(null);
          }}
          disabled={checking}
          placeholder="Schreib hier deinen Text auf Englisch…"
          rows={8}
          className="w-full text-[15px] leading-relaxed px-3.5 py-3 rounded-xl border-[1.5px] border-line bg-bg focus:bg-white focus:border-blue focus:shadow-[0_0_0_4px_rgba(0,113,227,0.12)] outline-none transition-all font-sans resize-y"
        />
        <div className="flex items-center justify-between mt-1.5 mb-4 text-[12px] text-ink-faint">
          <span>
            {wordCount} {wordCount === 1 ? "Wort" : "Wörter"} {wordCount < MIN_WORDS && `(mindestens ${MIN_WORDS} empfohlen)`}
          </span>
          {!allWordsUsed && <span className="text-amber font-semibold">Noch nicht alle Pflichtwörter verwendet</span>}
        </div>

        {error && (
          <div className="flex items-start gap-2 rounded-xl bg-red-light text-[#b8271b] text-[13px] px-3.5 py-3 mb-4">
            <AlertCircle size={16} className="shrink-0 mt-0.5" />
            {error}
          </div>
        )}

        {matches === null ? (
          <button
            onClick={handleCheck}
            disabled={checking || text.trim().length === 0}
            className="w-full rounded-full bg-gradient-to-r from-amber to-amber-dark hover:brightness-110 disabled:from-[#d1d1d6] disabled:to-[#d1d1d6] disabled:shadow-none disabled:cursor-not-allowed text-white font-semibold py-3.5 text-[15px] transition-all active:scale-[0.97] shadow-[0_10px_22px_-8px_rgba(232,161,46,0.5)] inline-flex items-center justify-center gap-2"
          >
            {checking ? (
              <>
                <Loader2 size={16} className="animate-spin" /> Wird geprüft…
              </>
            ) : (
              <>
                <Sparkles size={16} /> Prüfen
              </>
            )}
          </button>
        ) : (
          <div className="animate-fade-in">
            <div className="pt-4 border-t border-line-soft mb-3">
              <div className={"flex items-center gap-2 text-[15px] font-semibold mb-2.5 " + (matches.length === 0 ? "text-[#0d7a4f]" : "text-[#96690f]")}>
                <span
                  className={
                    "w-6 h-6 rounded-full flex items-center justify-center text-white shrink-0 shadow-sm " +
                    (matches.length === 0 ? "bg-gradient-to-br from-green to-green-dark" : "bg-gradient-to-br from-amber to-amber-dark")
                  }
                >
                  {matches.length === 0 ? <Check size={14} /> : <AlertCircle size={14} />}
                </span>
                {matches.length === 0 ? "Keine Fehler gefunden" : `${matches.length} ${matches.length === 1 ? "Hinweis" : "Hinweise"} gefunden`}
              </div>
              <div className="bg-bg rounded-xl p-3.5 text-[14.5px] leading-relaxed text-ink whitespace-pre-wrap">
                {renderHighlighted(text, matches)}
              </div>
            </div>

            {matches.length > 0 && (
              <div className="flex flex-col gap-2.5 mb-4">
                {matches.map((m, i) => (
                  <div key={i} className="rounded-xl border border-line-soft bg-card px-3.5 py-3">
                    <div className="text-[11px] font-bold uppercase tracking-wide text-amber mb-1">{m.ruleCategory}</div>
                    <div className="text-[13.5px] text-ink-soft mb-1.5">{m.message}</div>
                    {m.replacements.length > 0 && (
                      <div className="flex flex-wrap gap-1.5">
                        {m.replacements.map((r, ri) => (
                          <span key={ri} className="text-[12px] font-semibold px-2 py-0.5 rounded-full bg-green-light text-[#0d7a4f]">
                            {r}
                          </span>
                        ))}
                      </div>
                    )}
                  </div>
                ))}
              </div>
            )}

            <div className="flex gap-2">
              <button
                onClick={() => setMatches(null)}
                className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-full border border-line bg-card hover:bg-line-soft text-ink font-semibold py-3 text-[14px] transition-all active:scale-[0.97]"
              >
                <RefreshCw size={14} /> Weiter bearbeiten
              </button>
              <button
                onClick={handleFinish}
                className="flex-1 rounded-full bg-gradient-to-r from-blue to-blue-dark hover:brightness-110 text-white font-semibold py-3 text-[14px] transition-all active:scale-[0.97] shadow-[0_10px_22px_-8px_rgba(0,113,227,0.5)]"
              >
                Fertig
              </button>
            </div>
          </div>
        )}
      </div>
    </section>
  );
}

function renderHighlighted(text: string, matches: LanguageToolMatch[]): React.ReactNode {
  if (matches.length === 0) return text;
  const sorted = [...matches].sort((a, b) => a.offset - b.offset);
  const nodes: React.ReactNode[] = [];
  let cursor = 0;
  sorted.forEach((m, i) => {
    if (m.offset < cursor) return;
    if (m.offset > cursor) nodes.push(text.slice(cursor, m.offset));
    const end = m.offset + m.length;
    nodes.push(
      <mark key={i} title={m.shortMessage} className="bg-red-light text-[#b8271b] rounded px-0.5 not-italic">
        {text.slice(m.offset, end)}
      </mark>
    );
    cursor = end;
  });
  if (cursor < text.length) nodes.push(text.slice(cursor));
  return nodes;
}
